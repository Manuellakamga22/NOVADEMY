...
exports.createMessage = async ({ sender_id, receiver_id, content, blocked_content_detected }) => {
  // je cherche une conversation existante entre les deux users
  ...
  if (convRows.length > 0) {
    ...
    // je récupère l'announcement_id depuis un trial accepté entre ces deux users
    ...
    const [newConv] = await db.query(
      `INSERT INTO conversations (teacher_id, student_id, announcement_id) VALUES (?, ?, ?)`,
  ...
  const [result] = await db.query(
    `INSERT INTO messages (conversation_id, sender_id, receiver_id, content, blocked_content_detected)
     VALUES (?, ?, ?, ?, ?)`,
